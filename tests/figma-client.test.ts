...
describe("FigmaClient", () => {
  ...
  it("should pass depth when fetching file nodes", async () => {
    const client = new FigmaClient(TEST_TOKEN);
    let requestedUrl = "";
    ...
    try {
      await client.getFileNodes("file123", ["1:2"], undefined, 10);
      expect(requestedUrl).toContain("/files/file123/nodes");
      expect(requestedUrl).toContain("ids=1%3A2");
      expect(requestedUrl).toContain("depth=10");
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
