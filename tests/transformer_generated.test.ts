...
describe("inferSemanticRole", () => {
  ...
  it("should detect INPUT from name", () => {
    ...
    expect(inferSemanticRole({ id: "1", name: "search-bar", type: "FRAME" } as FigmaNode)).toEqual({ role: "INPUT", html: "input" });
  });
  ...
  it("should detect CARD from name", () => {
    ...
    expect(inferSemanticRole({ id: "1", name: "tile-product", type: "FRAME" } as FigmaNode)).toEqual({ role: "CARD", html: "article" });
  });
  ...
  it("should detect ICON from name", () => {
    ...
    expect(inferSemanticRole({ id: "1", name: "ico-close", type: "FRAME" } as FigmaNode)).toEqual({ role: "ICON", html: "svg" });
  });
  ...
  it("should detect MODAL from name", () => {
    expect(inferSemanticRole({ id: "1", name: "modal-dialog", type: "FRAME" } as FigmaNode)).toEqual({ role: "DIALOG", html: "dialog" });
    expect(inferSemanticRole({ id: "1", name: "popup-confirm", type: "FRAME" } as FigmaNode)).toEqual({ role: "DIALOG", html: "dialog" });
  });
  ...
  it("should detect INSTANCE type", () => {
    expect(inferSemanticRole({ id: "1", name: "Instance", type: "INSTANCE" } as FigmaNode)).toEqual({ role: "INSTANCE", html: "div" });
  });
...
describe("parseEffects", () => {
  it("should return null for undefined effects", () => {
  ...
  it("should parse inner shadow", () => {
    ...
    expect(result).not.toBeNull();
    expect(result.length).toBe(1);
    expect(result![0].type).toBe("inner-shadow");
    expect(result![0].offset).toEqual({ x: 0, y: 2 });
  });
  ...
  it("should parse background blur", () => {
    ...
    expect(result).not.toBeNull();
    expect(result.length).toBe(1);
    expect(result![0].type).toBe("backdrop-blur");
    expect(result![0].radius).toBe(20);
  });
...
describe("simplifyNode", () => {
  ...
  it("should include semantic role when detected", () => {
    ...
    expect(result.htmlTag).toBe("header");
  });
  ...
  it("should include layout info for auto-layout nodes", () => {
    ...
    expect(result.layout.padding).toBe(16);
  });
  ...
  it("should include text content for TEXT nodes", () => {
    ...
    expect(result.textStyle.size).toBe(24);
...
describe("buildComponentMap", () => {
  ...
  it("should include COMPONENT_SET nodes", () => {
    ...
    const result = buildComponentMap(node);
    expect(result["cs:1"]).toBeUndefined();
    expect(result["comp:1"]).toBeDefined();
...
describe("toCondensedFormat", () => {
  ...
  it("should include inferred layout for manual rows", () => {
    const node: FigmaNode = {
      id: "1:1",
      name: "Manual Row",
      type: "FRAME",
      children: [
        { id: "1:2", name: "A", type: "RECTANGLE", absoluteBoundingBox: { x: 0, y: 0, width: 40, height: 24 } },
        { id: "1:3", name: "B", type: "RECTANGLE", absoluteBoundingBox: { x: 56, y: 0, width: 40, height: 24 } },
      ],
    };
    const result = toCondensedFormat(node);
    expect(result).toContain("inferred-row");
    expect(result).toContain("inferred-gap:16");
    expect(result).toContain("confidence:high");
    expect(result).not.toContain("flex-row");
  });

  it("should preserve auto-layout over inferred layout", () => {
    const node: FigmaNode = {
      id: "1:1",
      name: "Auto Row",
      type: "FRAME",
    ...
    expect(result).toContain("flex-row");
    expect(result).toContain("gap:12");
    expect(result).not.toContain("inferred-row");
  });

  it("should not treat layoutMode NONE as auto-layout", () => {
    const node: FigmaNode = {
      id: "1:1",
      name: "Manual Column",
      type: "FRAME",
      ...
      children: [
        { id: "1:2", name: "A", type: "RECTANGLE", absoluteBoundingBox: { x: 0, y: 0, width: 40, height: 24 } },
        { id: "1:3", name: "B", type: "RECTANGLE", absoluteBoundingBox: { x: 0, y: 40, width: 40, height: 24 } },
      ],
    };
    const result = toCondensedFormat(node);
    expect(result).toContain("inferred-col");
  ...
  it("should handle variable bindings", () => {
    ...
    const result = toCondensedFormat(node, 0, 10, variableMap);
    expect(result).toContain("var(--colors-primary)");
  });
});

describe("toCondensedWithBudget", () => {
  it("should ignore token budget and include all nodes within default depth", () => {
    const node: FigmaNode = {
      id: "1:1",
      name: "Page",
      type: "FRAME",
    ...
    expect(result).toContain("Item19");
  });
  ...
  it("should not truncate deep trees to fit budget", () => {
    ...
    expect(result).toContain("Child49");
    expect(result).toContain("Grandchild4");
    expect(result).not.toContain("truncated");
  });
});
